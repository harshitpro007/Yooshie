...
class App {
  ...
  private async startapp() {
    fetchSecrets().then(async () => {
      this.template();
      this.Port = process.env.TASK_PORT;
  ...
  private callback = () => {
    logger.info(
      ` Hapi server swagger is running this URL :- ${SERVER.APP_URL}:${SERVER.TASK_PORT}/${SERVER.TASK_MICROSERVICE_URL}/documentation`
    );
