      ...
      validate: async (
        ...
        try {
          ...
          if (!isValidApiKey) {
            return {
              isValid: false,
              credentials: { accessToken: accessToken, tokenData: {} },
            };
          } else {
            try {
              const payload = await axiosService.getData({
                url: process.env.AUTH_APP_URL + SERVER.VERIFY_AUTH_TOKEN,
                body: {},
              ...
              return {
                ...
                credentials: {
                  ...
                  tokenData: payload.credentials.tokenData,
                },
              };
            } catch (e) {
              ...
              if (e.type == "BAD_TOKEN") {
                return Promise.reject(
                  responseHandler.sendError(request, MESSAGES.ERROR.BAD_TOKEN)
                );
              }
            }
          }
      ...
      validate: async (
        ...
        try {
          ...
          if (!isValidApiKey) {
            ...
            try {
              const payload = await axiosService.getData({
                url: process.env.AUTH_APP_URL + SERVER.VERIFY_COMMON_AUTH_TOKEN,
                body: {},
                auth: "Bearer " + accessToken,
              });
              return {
    ...
    server.auth.strategy("DoubleAuth", "bearer-access-token", {
      ...
      validate: async (request: Request, accessToken, h: ResponseToolkit) => {
        ...
        if (checkFunction) {
          return {
            isValid: true,
            credentials: { token: accessToken, userData: {} },
          };
        }
      },
    });
