 ...
 * @description This method checks the token and returns the decoded data when token is valid in all respect
 */
const validateResetToken = async function (token: string, request?: Request, auth: boolean = true): Promise<JwtPayload> {
	try {
		const cert = await readPublicKey();
		return await Jwt.verify(token,cert);
		...
		if (error && error.name === "TokenExpiredError" && auth) return Promise.reject(MESSAGES.ERROR.RESET_SESSION_EXPIRED);
		if (error && error.name === "TokenExpiredError" && !auth) return Promise.reject(MESSAGES.ERROR.RESET_TOKEN_EXPIRED);
		// throws error if the token has not been encrypted by the private key
		return Promise.reject(responseHandler.sendError(request, MESSAGES.ERROR.BAD_RESET_TOKEN));
	}
};
