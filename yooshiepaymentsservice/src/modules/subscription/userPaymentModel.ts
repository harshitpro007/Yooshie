...
import { DB_MODEL_REF, PAYMENT_STATUS, SUBSCRIPTION_STATUS, SUB_TYPE } from "@config/constant";
...
const userPaymentSchema: Schema = new mongoose.Schema({
	...
	cardDetails: {
		...
		lastFourDigit: { type: String, required: false }
	},
	subscriptionId: {
		type: String,
		required: false,
	...
	subscriptionStatus: {type: String, required: false, enum: Object.values(SUBSCRIPTION_STATUS)},
...
export const payments: Model<IUserPayment> = model<IUserPayment>(DB_MODEL_REF.USER_PAYMENTS, userPaymentSchema);
