...
class App {
  ...
  private async startapp() {
    fetchSecrets().then(async () => {
      this.template();
      this.Port = process.env.PAY_PORT;
  ...
  private callback = () => {
    logger.info(
      ` Hapi server swagger is running this URL :- ${SERVER.APP_URL}:${SERVER.PAY_PORT}/${SERVER.PAYMENT_MICROSERVICE_URL}/documentation`
    );
